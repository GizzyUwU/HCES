...
export default new Elysia()
  ...
  .get(
    ...
    async ({ set, client, params, query }) => {
      try {
        const res = await client.userProjects(params, query);
        if (!res)
          throw new APIError({
            status: 500,
        ...
        logger.error("Failed getting ft user projects data", {
          error: err,
          params,
        });
        throw new APIError({
    ...
    },
    {
      ...
      params: FTTypes["ListUserProjectsParams"],
      query: FTTypes["ListUserProjectsQueryParams"],
      response: {
        200: FTTypes["ListUserProjectsResponse"],
      },
    },
  );
